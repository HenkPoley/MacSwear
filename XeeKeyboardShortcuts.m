...
{
	XeeAction *action=[self actionForEvent:event ignoreMenuItems:YES];
	if(action&&[action perform:event]) return YES;
	return NO;
}

-(XeeAction *)actionForEvent:(NSEvent *)event { return [self actionForEvent:event ignoreMenuItems:NO]; }

-(XeeAction *)actionForEvent:(NSEvent *)event ignoreMenuItems:(BOOL)ignoremenu
{
	NSEnumerator *enumerator=[actions objectEnumerator];
	XeeAction *action;
	...
	while(action=[enumerator nextObject])
	{
		...
		{
			if([key matchesEvent:event]) return action;
		}
	}
	return nil;
}
...
-(id)initWithTitle:(NSString *)acttitle identifier:(NSString *)ident selector:(SEL)selector target:(id)acttarget defaultShortcut:(XeeKeyStroke *)defshortcut
{
	if(self=[super init])
	{
...
{
	if(self=[self initWithTitle:[menuitem title] identifier:nil selector:[menuitem action] target:[menuitem target] defaultShortcut:[XeeKeyStroke keyFromMenuItem:menuitem]])
	{
...
-(void)setShortcuts:(NSArray *)shortcutarray
{
	...
	else
	{
		...
		[[NSUserDefaults standardUserDefaults] setObject:dictionaries forKey:identifier];
	}
...
{
	NSArray *dictionaries=[[NSUserDefaults standardUserDefaults] arrayForKey:identifier];
...
{
	return [[[XeeAction alloc] initWithTitle:acttitle identifier:nil selector:selector target:nil defaultShortcut:nil] autorelease];
}

+(XeeAction *)actionWithTitle:(NSString *)acttitle identifier:(NSString *)ident selector:(SEL)selector
{
	return [[[XeeAction alloc] initWithTitle:acttitle identifier:ident selector:selector target:nil defaultShortcut:nil] autorelease];
}

+(XeeAction *)actionWithTitle:(NSString *)acttitle identifier:(NSString *)ident selector:(SEL)selector defaultShortcut:(XeeKeyStroke *)defshortcut
{
	return [[[XeeAction alloc] initWithTitle:acttitle identifier:ident selector:selector target:nil defaultShortcut:defshortcut] autorelease];
}

+(XeeAction *)actionWithTitle:(NSString *)acttitle identifier:(NSString *)ident
{
	return [[[XeeAction alloc] initWithTitle:acttitle identifier:ident selector:0 target:nil defaultShortcut:nil] autorelease];
}
